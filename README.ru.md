...
Версия уже подготовлена: `0.3.1`. После коммита workflow и настройки npm:
...
Проверки управляемых файлов допускают различия LF/CRLF, в том числе при
обновлении со старых отпечатков для любого из этих форматов. Содержательные
правки по-прежнему вызывают CT301/CT109. Обновление записывает шаблон пакета,
не меняя настройки Git. Если старый отпечаток отсутствует или не соответствует
исходному файлу, требуется ручной разбор. Изменения после просмотра плана
проверяются побайтово, включая переносы строк.
